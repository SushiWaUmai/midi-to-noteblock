...
schem_dir = os.path.join(home_dir, ".minecraft/config/worldedit/schematics")
...
def create_note_block(reg, width, note, instrument_block, start):
    ...
    note_block = Block.create(f'minecraft:note_block[note={str(note.pitch - 54)}]')
...
def generate_schematic(midi_path, schem_name, width=7, start=5):
    ...
    target_path = os.path.join(schem_dir, schem_name)
    print(target_path)
...
def main():
    generate_schematic("/mnt/misc/onedrive/Audio/MIDI/A_Cruel_Angels_Thesis.mid", "test.schem")
